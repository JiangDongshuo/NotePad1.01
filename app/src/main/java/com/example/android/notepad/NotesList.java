...
public class NotesList extends ListActivity {
   ...
   // private ListView listView=(ListView) findViewById(android.R.id.list);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.listback);
        ...
        editText.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                System.out.println("搜索框文本变化了");
                System.out.println(editText.getText().toString());
                String [] searchContent={editText.getText().toString()+"%"};
                Cursor cursor = managedQuery(
                        getIntent().getData(),            // Use the default content URI for the provider.
                        PROJECTION,                       // Return the note ID and title for each note.
                        "title like ?",                             // No where clause, return all records.
                        searchContent,                             // No where clause, therefore no where column values.
                        NotePad.Notes.DEFAULT_SORT_ORDER  // Use the default sort order.
                );
                ListView listView=(ListView) findViewById(android.R.id.list);
                String[] dataColumns = { NotePad.Notes.COLUMN_NAME_TITLE,NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE} ;
                int[] viewIDs = { R.id.head1,R.id.time1 };
                SimpleCursorAdapter adapter=new SimpleCursorAdapter(
                       listView.getContext() ,R.layout.newnoteslist_item,cursor,dataColumns,viewIDs
                );
        ...
        SimpleCursorAdapter adapter
            = new SimpleCursorAdapter(
                      ...
                      R.layout.newnoteslist_item,          // Points to the XML for a list item
